...
/*
Otherwise, the method finds the RCB in the request queue whose cylinder is closest to the current cylinder.
 If there are multiple requests with the closest cylinder, then the method picks the request among these that
  has the earliest arrival_timestamp. The method then removes the RCB of the selected request from the request
...
//---------------------------------------------------------------------------------------------
      /*Otherwise, if the scan direction is 1 and there are requests with cylinders larger than the current cylinder,
       ...
       */
         if(scan_direction ==1){
            ...
            int hiclosest = abs(current_cylinder - hicyl[0].cylinder),pos;
                ...
                *queue_cnt-=1;
                ...
                /*Otherwise, if the scan direction is 1
                 and there are no requests with cylinders larger than the current cylinder,
        ...
        /*Otherwise, if the scan direction is 0 and there are requests with cylinders smaller than the current cylinder,
         the method picks the one among these whose cylinder is closest to the current cylinder. Otherwise, if the scan direction
          is 0 and there are requests with cylinders larger than the current cylinder, the method picks the request whose cylinder
...
/*
 ...
 int main(){
     ...
     struct RCB rq0 = {51,1,90,53,51}; 
     rq[0] =  rq0;
     struct RCB rq1 = {52,3,15,54,52};
     struct RCB rq2 = {53,4,45,55,53};
     struct RCB rq3 = {54,7,28,56,54};
     rq[1] = rq1;
     rq[2] = rq2;
     rq[3] = rq3;

     struct RCB help = handle_request_completion_sstf(rq,&qcnt,c_cyl);
     printf("hi");
 }
